...
function fulfillmentToStepIndex(
  ...
  if (fulfillment === "cancelled" || fulfillment === "returned" || status === "cancelled")
    return -1;
...
const statusMessage = (
  ...
  if (fulfillment === "shipped" || forwarding === "forwarded")
    return "Shipped — Tracking available";
...
export function OrderTimeline({
  ...
  const activeIndex = fulfillmentToStepIndex(
    orderStatus,
    fulfillmentStatus,
    forwardingStatus,
    !!hasTracking,
    ...
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground px-1">
          {statusMessage(orderStatus, fulfillmentStatus, forwardingStatus, paymentStatus)}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={cn(compact ? "space-y-0" : "space-y-1")}>
      <p className={cn("text-xs font-medium mb-2", compact ? "hidden sm:block" : "")}>
        {statusMessage(orderStatus, fulfillmentStatus, forwardingStatus, paymentStatus)}
      </p>
          ...
          return (
            <div key={step.key} className="flex flex-1 flex-col items-center gap-1">
              <div className="flex w-full items-center">
                {idx > 0 && (
                  <div
                    className={cn("h-0.5 flex-1", isDone || isActive ? "bg-brand" : "bg-border")}
                  />
                )}
                <div
                  className={cn(
              ...
              <span
                className={cn(
                  ...
                  isActive ? "text-brand" : isDone ? "text-foreground" : "text-muted-foreground",
                )}
              >
                {step.label}
              </span>
              {!compact && (
                <span className="text-[11px] text-muted-foreground text-center leading-tight hidden sm:block">
                  {idx === 0 && createdAt
                    ...
                    : idx === 1 && forwardedAt
                      ? new Date(forwardedAt).toLocaleDateString()
                      : idx === 3 && shippedAt
