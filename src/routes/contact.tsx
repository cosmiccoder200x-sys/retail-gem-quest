...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: `Contact ${siteConfig.name} — email ${siteConfig.email}, support form and FAQ.` },
...
function ContactPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      <p className="mt-2 text-muted-foreground">Questions about an order, shipping, or a product? We're here.</p>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[1fr_300px]">
        <form onSubmit={submit} className="rounded-3xl bg-white p-6 ring-1 ring-border space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Messages are stored securely. Email delivery is configured via Supabase; add SMTP in project settings to enable notifications.</p>
          </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl bg-card p-4 ring-1 ring-border">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Email: {siteConfig.email}<br />Phone: {siteConfig.phone}<br />{siteConfig.address}</p>
            <p className="mt-2 text-xs text-muted-foreground">Edit contacts in <code>src/lib/site.ts</code>.</p>
          </div>
          <div className="rounded-2xl bg-brand-soft p-4 ring-1 ring-border">
            <p className="text-sm font-medium">Need quick answers?</p>
            <Link to="/faq" className="text-sm text-brand hover:underline">Visit FAQ →</Link>
            <span className="mx-2 text-muted-foreground">·</span>
            <Link to="/track" className="text-sm text-brand hover:underline">Track order →</Link>
          </div>
        </div>
      </div>
    </div>
