...
export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({
    meta: [{ title: "Admin — GullyGadget" }, { name: "robots", content: "noindex, nofollow" }],
  }),
  component: Admin,
});
...
function AdminOrders() {
  ...
  const { data: orders, isPending } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filter === "cod") q = q.eq("payment_method", "cod");
        ...
        q = q.eq("payment_status", "pending").eq("payment_method", "online");
  ...
  const bulkForward = useMutation({
    mutationFn: async () => {
      ...
      if (fail > 0)
        throw new Error(
          `Bulk: ${ok} forwarded, ${fail} failed. ${results
            .filter((r) => !r.ok)
            .map((r) => r.error)
            .join("; ")}`,
        );
    ...
    <div className="mt-6 space-y-4">
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 rounded-2xl bg-brand-soft p-3">
          ...
          <Button
            ...
            onClick={() => bulkForward.mutate()}
            disabled={bulkForward.isPending}
          >
            Forward selected
          </Button>
      ...
      {(orders ?? []).map((o) => (
        <AdminOrderCard
          ...
          onSave={(patch: Record<string, unknown>) => update.mutate({ id: o.id, patch })}
          onForward={(supplierId?: string) => forward.mutate({ id: o.id, supplierId })}
        />
      ))}
    </div>
  );
...
function AdminOrderCard({
  ...
  const [expectedDelivery, setExpectedDelivery] = useState(
    (order.expected_delivery_date as string) ?? "",
  );
  const [failureReason, setFailureReason] = useState(
    (order.forwarding_failure_reason as string) ?? "",
  );
  const [showHistory, setShowHistory] = useState(false);
  const isPaidOrCod = order.payment_status === "paid" || order.payment_method === "cod";
  const canForward =
    order.forwarding_status === "pending" && isPaidOrCod && order.status !== "cancelled";
    ...
    <div className="rounded-3xl bg-white p-4 ring-1 ring-border">
        ...
        {order.supplier_id && (
          <span className="text-xs bg-secondary px-2 py-0.5 rounded-full">
            {suppliers.find((s) => s.id === order.supplier_id)?.name ??
              String(order.supplier_id).slice(0, 6)}
          </span>
        )}
        <span className="text-xs text-muted-foreground">
          Sub {formatINR(Number(order.subtotal ?? 0))} · Disc{" "}
          {formatINR(Number(order.discount_amount ?? 0))}
          {order.coupon_code ? ` (${order.coupon_code})` : ""} · Ship{" "}
          {formatINR(Number(order.shipping ?? 0))}
        </span>
        <div className="ml-auto flex items-center gap-2">
          <Select value={String(order.status)} onValueChange={(v) => onSave({ status: v })}>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {canForward ? (
          <Button
            ...
            onClick={() => onForward(supplierId || undefined)}
          >
            Forward to supplier
          </Button>
          ...
          <span className="text-xs text-success font-medium">
            ✓ Forwarded{" "}
            {order.forwarded_at
              ? `on ${new Date(order.forwarded_at as string).toLocaleDateString()}`
              : ""}
          </span>
        ) : !isPaidOrCod ? (
        ...
        {order.forwarding_status === "failed" && (
          <span className="text-xs text-destructive">
            Failed
            {order.forwarding_failure_reason ? `: ${String(order.forwarding_failure_reason)}` : ""}
          </span>
        )}
      </div>
      {order.forwarding_status === "failed" && (
        <div className="mt-2 flex gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => onSave({ forwarding_failure_reason: failureReason || null })}
          >
            Save reason
          </Button>
          <Button size="sm" onClick={() => onForward(supplierId || undefined)}>
            Retry
          </Button>
        </div>
      )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(v) => {
            if (v === "cancelled" && !confirm("Cancel this order? This will stop fulfillment."))
              return;
        ...
        {order.shipped_at && (
          <span className="text-xs text-muted-foreground">
            Shipped {new Date(order.shipped_at as string).toLocaleDateString()}
          </span>
        )}
        {order.delivered_at && (
          <span className="text-xs text-muted-foreground">
      ...
      <div className="mt-2 flex flex-wrap justify-between gap-2">
        <Button size="sm" variant="ghost" onClick={() => setShowHistory((v) => !v)}>
          {showHistory ? "Hide history" : "Show history & notifications"}
        </Button>
          ...
          <Button
            ...
            onClick={() => {
              if (url && !isValidTrackingUrl(url)) {
                toast.error("Tracking URL must start with https://");
                return;
              }
              onSave({
      ...
      {showHistory && (
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl bg-muted/30 p-3">
            ...
            {(history ?? []).length === 0 ? (
              <p className="text-xs text-muted-foreground mt-1">No history yet.</p>
            ) : (
              (history ?? []).map((h: any) => (
                <div key={h.id} className="mt-2 text-xs flex justify-between gap-2">
                  <span>
                    {h.previous_status ?? "—"} → {h.new_status} / {h.previous_fulfillment ?? "—"} →{" "}
                    {h.new_fulfillment}
                  </span>
          ...
          <div className="rounded-xl bg-muted/30 p-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              Provider: none until configured (see order_notifications table comment).
            </p>
            {(notifications ?? []).length === 0 ? (
              <p className="text-xs text-muted-foreground mt-1">No events yet.</p>
            ) : (
              (notifications ?? []).map((n: any) => (
                <div key={n.id} className="mt-1 flex justify-between text-xs">
                  <span>
                    {n.event} — {n.status}
                    {n.error ? `: ${n.error.slice(0, 60)}` : ""}
                  </span>
                  <span className="text-muted-foreground">
...
function AdminProducts() {
  ...
  const { data: salesMap } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const r of data ?? []) {
        ...
        cur.revenue += Number(r.unit_price ?? 0) * r.quantity;
  ...
  const filtered = (() => {
    ...
    // Sorting
    if (sort === "units")
      list.sort((a, b) => (salesMap?.get(b.id)?.units ?? 0) - (salesMap?.get(a.id)?.units ?? 0));
    else if (sort === "revenue")
      list.sort(
        (a, b) => (salesMap?.get(b.id)?.revenue ?? 0) - (salesMap?.get(a.id)?.revenue ?? 0),
      );
      ...
      {lowStock.length > 0 && (
        <div className="rounded-2xl bg-amber-50 p-4 ring-1 ring-amber-200">
          <p className="text-sm font-bold text-amber-800">
            Low stock — {lowStock.length} product(s) ≤ 5 units
          </p>
          <ul className="mt-2 space-y-1 text-xs text-amber-700">
            ...
            {lowStock.length > 8 && (
              <li className="text-muted-foreground">+ {lowStock.length - 8} more</li>
            )}
          </ul>
        </div>
      )}
            ...
            <tbody>
              {filtered.map((p) => {
                ...
                return (
                  <tr
                    key={p.id}
                    className={`border-t border-border ${p.is_active === false ? "opacity-50" : ""} ${isLow ? "bg-amber-50/30" : ""}`}
                  >
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {p.slug} · {formatINR(Number(p.price))}
                          </p>
                        </div>
                      </div>
                    </td>
    ...
    <div className="mt-6 space-y-4">
      ...
      <div className="rounded-3xl bg-white ring-1 ring-border">
        ...
        {(categories ?? []).map((cat) => (
          <div
            key={cat.id}
            className={`flex items-center gap-4 border-b border-border p-4 last:border-0 ${!cat.is_active ? "opacity-50" : ""}`}
          >
            <div className="flex-1 min-w-0">
              <p className="font-bold">
                {cat.name}{" "}
                {!cat.is_active && (
                  <span className="ml-2 text-xs bg-muted px-2 py-0.5 rounded-full">Archived</span>
                )}
              </p>
              <p className="text-xs text-muted-foreground">
                {cat.slug} · Sort: {cat.sort_order}
              </p>
              {cat.description && (
            ...
            <div className="flex gap-1 shrink-0">
              ...
              <Button
                size="sm"
                variant={cat.is_active ? "ghost" : "default"}
                onClick={() => archive.mutate(cat.id)}
                className={cat.is_active ? "text-destructive" : ""}
              >
...
function AdminReviews() {
  ...
  const { data: reviews, isPending } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (q.trim()) query = query.or(`title.ilike.%${q.trim()}%,comment.ilike.%${q.trim()}%`);
  ...
  const toggleVisible = useMutation({
    mutationFn: async ({ id, is_visible }: { id: string; is_visible: boolean }) => {
      const { error } = await supabase.from("product_reviews").update({ is_visible }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {(reviews ?? []).map((r: any) => (
        <div
          key={r.id}
          className={`rounded-2xl bg-white p-4 ring-1 ring-border ${!r.is_visible ? "opacity-60" : ""}`}
        >
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="font-bold">{r.products?.name ?? r.product_id.slice(0, 8)}</span>
            <span className="text-muted-foreground">
              by {r.profiles?.full_name ?? r.user_id.slice(0, 8)}
            </span>
            <span className="bg-secondary px-2 py-0.5 rounded-full">{r.rating} ★</span>
            {r.verified_purchase && (
              <span className="bg-success/10 text-success px-2 py-0.5 rounded-full">Verified</span>
            )}
            {!r.is_visible && (
...
function AdminCoupons() {
  ...
  const { data: coupons } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filter === "active")
        ...
        rows = rows.filter((c: any) => c.expires_at && new Date(c.expires_at) <= now);
      else if (filter === "upcoming")
        rows = rows.filter((c: any) => c.starts_at && new Date(c.starts_at) > now);
  ...
  return (
    <div className="mt-6 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setDraft({ ...draft, maximum_discount: e.target.value })}
                placeholder="e.g. 500"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setDraft({ ...draft, min_order_total: e.target.value })}
              />
            </div>
            <div>
      ...
      <div className="rounded-3xl bg-white ring-1 ring-border">
        {(coupons ?? []).length === 0 && (
          <p className="p-6 text-sm text-muted-foreground">No coupons.</p>
        )}
        {(coupons ?? []).map((c: any) => (
          <div
            key={c.id}
            ...
            <div className="min-w-0 flex-1">
              <p className="font-mono font-bold">
                ...
                <span className="text-xs font-normal text-muted-foreground">
                  ...
                  {c.maximum_discount ? ` (max ₹${c.maximum_discount})` : ""}
                </span>{" "}
                {!c.is_active && (
                  <span className="ml-2 text-xs bg-muted px-2 py-0.5 rounded-full">Disabled</span>
                )}
              </p>
              <p className="text-xs text-muted-foreground">
                Min ₹{c.min_order_total ?? 0} · Used {c.used_count}/{c.max_uses ?? "∞"} · Per user{" "}
                {c.per_user_limit ?? "∞"} · Discount{" "}
                {formatINR(couponStats?.get(c.code)?.discount ?? 0)}{" "}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setDraft({
                    ...
                    min_order_total: c.min_order_total ? String(c.min_order_total) : "",
...
function AdminShipping() {
  ...
  const [draft, setDraft] = useState({
    base_shipping_charge: 79,
    free_shipping_threshold: 999,
    cod_min_order_value: 0,
  });
  const [loadedId, setLoadedId] = useState<string | null>(null);
  if (cfg && loadedId !== cfg.id) {
    setDraft({
      base_shipping_charge: Number(cfg.base_shipping_charge),
      free_shipping_threshold: Number(cfg.free_shipping_threshold),
      cod_min_order_value: Number(cfg.cod_min_order_value ?? 0),
    });
    setLoadedId(cfg.id);
  }
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("shipping_config")
        .update({
          base_shipping_charge: draft.base_shipping_charge,
          free_shipping_threshold: draft.free_shipping_threshold,
          cod_min_order_value: draft.cod_min_order_value,
        })
        .eq("id", cfg.id);
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["admin-shipping-config"] });
  ...
  return (
    <div className="mt-6 max-w-lg space-y-4 rounded-3xl bg-white p-6 ring-1 ring-border">
      ...
      <div>
        ...
        <Input
          type="number"
          value={draft.base_shipping_charge}
          onChange={(e) => setDraft({ ...draft, base_shipping_charge: Number(e.target.value) })}
        />
      </div>
      <div>
        ...
        <Input
          ...
          onChange={(e) => setDraft({ ...draft, free_shipping_threshold: Number(e.target.value) })}
        />
      ...
      <div>
        <Label>COD min order ₹ (optional)</Label>
        <Input
          type="number"
          value={draft.cod_min_order_value}
          onChange={(e) => setDraft({ ...draft, cod_min_order_value: Number(e.target.value) })}
        />
      </div>
      <div className="rounded-xl bg-muted p-3 text-xs">
        Preview: Subtotal ₹500 → Shipping{" "}
        {500 >= draft.free_shipping_threshold ? "Free" : formatINR(draft.base_shipping_charge)} ·
        Subtotal ₹{draft.free_shipping_threshold} → Free
      </div>
      <Button onClick={() => save.mutate()} disabled={save.isPending}>
        Save
      </Button>
      ...
      {(suppliers ?? []).map((s) => (
        ...
        >
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              {s.platform} {s.contact_email ? `· ${s.contact_email}` : ""}
            </p>
          </div>
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${s.is_active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}
          >
