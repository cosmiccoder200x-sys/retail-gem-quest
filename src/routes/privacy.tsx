...
export const Route = createFileRoute("/privacy")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: `Privacy Policy for ${siteConfig.name} — what data we collect and how we use it.`,
      },
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Template — have counsel review before launch. Business details are editable in{" "}
      ...
      <div className="mt-8 space-y-6 leading-relaxed text-sm">
        ...
        <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
          ...
          <li>
            Orders: shipping address, items, payment status (not card/UPI — handled by Razorpay).
          </li>
        ...
        <p className="text-muted-foreground">
          We share order data only with services needed to run the store (e.g., Supabase for
          hosting, Razorpay for payments, couriers for delivery). No sale of personal data.
        </p>
        <h2 className="font-display text-lg uppercase">Retention & rights</h2>
        <p className="text-muted-foreground">
          You may request access or deletion of your account data via {siteConfig.email}. Contact
          messages are retained for support history and accessible to admins only.
        </p>
        <h2 className="font-display text-lg uppercase">Contact</h2>
        <p className="text-muted-foreground">
          Questions: {siteConfig.email}, {siteConfig.address}
        </p>
      </div>
    </div>
  );
